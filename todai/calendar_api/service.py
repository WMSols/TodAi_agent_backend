...
from todai.calendar_api.recurrence import expand_daily_occurrences, expand_weekly_occurrences
        ...
        if rec_ids:
            try:
                rec_rows = (
                    ...
                    .select(
                        "id, frequency, weekly_mode, skip_days, repeat_weeks, repeat_days, "
...
def _recurrence_api(row: dict[str, Any], tz: str) -> dict[str, Any]:
    ...
    if freq == "daily":
        out["repeat_days"] = int(row.get("repeat_days") or row.get("repeat_weeks") or 7)
    else:
        out["repeat_weeks"] = int(row.get("repeat_weeks") or 12)
        out["skip_days"] = [int(x) for x in skip]
    return out
...
def _create_event(user_id: str, body: dict[str, Any]) -> dict[str, Any]:
    ...
    if rec.get("enabled"):
        frequency = str(rec.get("frequency") or "weekly").lower().strip()
        if frequency not in ("daily", "weekly"):
            frequency = "weekly"
        anchor_start_utc = local_naive_to_utc(body["start"], tz).isoformat()
        anchor_end_utc = local_naive_to_utc(body["end"], tz).isoformat()

        if frequency == "daily":
            days = int(rec.get("repeat_days") or 7)
            occurrences = expand_daily_occurrences(start, end, repeat_days=days)
            rec_row: dict[str, Any] = {
                ...
                "repeat_weeks": 1,
                "repeat_days": days,
                "anchor_start_at": anchor_start_utc,
                "anchor_end_at": anchor_end_utc,
            }
        else:
            skip_raw = rec.get("skip_days")
            if skip_raw is None or skip_raw == []:
                anchor_dow = start.weekday()
                skip = {d for d in range(7) if d != anchor_dow}
            else:
                skip = {int(x) for x in skip_raw}
            weeks = int(rec.get("repeat_weeks") or 12)
            occurrences = expand_weekly_occurrences(
                start, end, repeat_weeks=weeks, skip_days=skip
            )
            rec_row = {
                "user_id": db_uid,
                "frequency": "weekly",
        ...
        try:
            rec_ins = client.table("calendar_recurrence").insert(rec_row).execute()
        except Exception as exc:
            if "repeat_days" in str(exc).lower() or "42703" in str(exc):
                rec_row.pop("repeat_days", None)
                rec_ins = client.table("calendar_recurrence").insert(rec_row).execute()
            elif "daily" in str(exc).lower() and "frequency" in str(exc).lower():
                raise ValueError(
                    "Daily recurrence requires DB migration docs/supabase/003_calendar_recurrence_daily.sql"
                ) from exc
            else:
                raise
...
def update_event(
    ...
    *,
    ...
    return _update_event(user_id, event_id, body, update_series=update_series)


def _series_recurrence_meta(client: Any, recurrence_id: str, tz: str) -> dict[str, Any] | None:
    rr = (
        client.table("calendar_recurrence")
        .select("*")
        .eq("id", recurrence_id)
    ...
    if rr.data:
        return _recurrence_api(rr.data[0], tz)
    return None
...
def _update_event(
    ...
    *,
    ...
    profile = SupabaseProfileRepository(ctx)
    tz = profile.tz_name()
    now_iso = datetime.now(__import__("datetime").timezone.utc).isoformat()

    anchor = (
        client.table("calendar_events")
        .select("id, recurrence_id, start_at, end_at")
        .eq("id", event_id)
        .eq("user_id", db_uid)
        .is_("deleted_at", "null")
        .limit(1)
    ...
    rid = anchor_row.get("recurrence_id")

    if update_series and rid:
        series_rows = (
            client.table("calendar_events")
        ...
        rows = list(series_rows.data or [])
        if not rows:
            raise LookupError("event not found")

        shared: dict[str, Any] = {"updated_at": now_iso}
        if "title" in body:
            shared["title"] = body["title"]
        if "description" in body:
            shared["description"] = body["description"]
        if "kind" in body:
            shared["kind"] = body["kind"]
        if "location" in body:
            shared["location"] = body["location"]
        if "all_day" in body:
            shared["all_day"] = body["all_day"]

        new_start_local = _parse_local_dt(body["start"]) if "start" in body else None
        new_end_local = _parse_local_dt(body["end"]) if "end" in body else None
        duration = None
        start_time = None
        if new_start_local and new_end_local:
            if new_end_local <= new_start_local:
                raise ValueError("end must be after start")
            duration = new_end_local - new_start_local
            start_time = new_start_local.time()

        updated_rows: list[dict[str, Any]] = []
        for row in rows:
            patch = dict(shared)
            if start_time is not None and duration is not None:
                local_start = _parse_local_dt(
                    utc_to_local_naive_str(parse_ts(row["start_at"]), tz)
                )
                occ_start = datetime.combine(local_start.date(), start_time)
                occ_end = occ_start + duration
                patch["start_at"] = local_naive_to_utc(
                    occ_start.strftime("%Y-%m-%dT%H:%M:%S"), tz
                ).isoformat()
                patch["end_at"] = local_naive_to_utc(
                    occ_end.strftime("%Y-%m-%dT%H:%M:%S"), tz
                ).isoformat()
            upd = (
                client.table("calendar_events")
                .update(patch)
            ...
            if upd.data:
                updated_rows.append(upd.data[0])

        rec = _series_recurrence_meta(client, str(rid), tz)
        with user_store(user_id) as store:
            ver = _bump_schedule_version(store)
        primary = next((r for r in updated_rows if str(r["id"]) == event_id), updated_rows[0])
        return {
            "ok": True,
            "event": _event_api_row(primary, tz, rec),
    ...
    if not upd.data:
        raise LookupError("event not found")
    row = upd.data[0]
    rec = _series_recurrence_meta(client, str(row["recurrence_id"]), tz) if row.get("recurrence_id") else None
    with user_store(user_id) as store:
        ver = _bump_schedule_version(store)
    return {
        "ok": True,
        "event": _event_api_row(row, tz, rec),
        "updated": 1,
        "scope": "occurrence",
        "schedule_version": ver,
    }
...
def _delete_event(user_id: str, event_id: str, *, delete_series: bool) -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "deleted": removed,
        "scope": "series" if delete_series and rid else "occurrence",
        "schedule_version": ver,
    }
